...
function getformattedLocalTime(area)
  if area == nil then
    return "الوقت في أي مكان أبدا"
  end

  lat,lng,acc = get_latlong(area)
  if lat == nil and lng == nil then
    return 'يبدو أن في "'..area..'" they do not have a concept of time.'
  end
  local localTime, timeZoneId = get_time(lat,lng)

  return "📍 الــتوقـيتَ الَمحـْليِ فـيَ  "..timeZoneId.." is: ".. os.date(dateFormat,localTime) 
end
...
return {
  ...
  usage = "وقت [area]: Displays the local time in that area",
  patterns = {"^[!#/]وقت (.*)$"}, 
  run = run
}
