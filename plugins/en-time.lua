...
function getformattedLocalTime(area)
  if area == nil then
    return "Time anywhere never"
  end

  lat,lng,acc = get_latlong(area)
  if lat == nil and lng == nil then
    return 'It seems that in the "'..area..'" they do not have a concept of time.'
  end
  local localTime, timeZoneId = get_time(lat,lng)

  return "Timing the local at"..timeZoneId.." is: ".. os.date(dateFormat,localTime) 
end
